...
class IDAStarPathFinding<Graph: IGraph> {
    ...
    func findPath_ID_AStar() -> [Node]? {
        var f_bound = graph.H(node0, target)
        path = [node0]
        pathS.insert(node0)
        
        repeat {
            print("\tTry f_bound=\(f_bound) ")
            let (found, f_bound1) = _search(n: node0, g_n: 0, f_bound: f_bound)
            if found {
    ...
    -> (found: Bool, min_bound: Graph.WeightType?) {
        ...
        let A = graph.adjacentNodes(with: n)
    ...
    //        let f_x = g_x + graph.H(x, target)
            let (found, min_bound1) = _search(n: x, g_n: g_x,
                                              f_bound: f_bound)
            if found {
